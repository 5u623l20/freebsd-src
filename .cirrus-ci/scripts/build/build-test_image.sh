...
CONFIG_BASE=${WORKSPACE}/.cirrus-ci/scripts/build/config
...
do
	tar Jxf ${WORKSPACE}/release/${ARTIFACT_DEST}/${f}.txz -C ufs
done
...
ls -la
rm -f ufs.img
chflags -R noschg ufs &
zstd -19 -T0 --rm -q --no-progress ${OUTPUT_IMG_NAME}
rm -rf ufs
